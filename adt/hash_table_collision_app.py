...
class HashTable:
    ...
    def insert(self, title, author):
        ...
        for item in self.table[index]:
            if item[0] == title:
                return f"Error: Book '{title}' already exists."
    ...
    def search(self, title):
        ...
        for k, v in self.table[index]:
            if k == title:
                return f"Found book '{title}' by '{v}' at index {index}."
        return f"Book '{title}' not found."
    ...
    def traverse(self):
        ...
        for index, items in enumerate(self.table):
            if items:
                values = ', '.join([f"Title '{title}', Author '{author}'" for title, author in items])
                contents.append(f"Index {index}: {values}")
            else:
...
class HashTableGUI:
    def __init__(self, root):
        ...
        self.working_text.insert(tk.END, "This code implements a hash table using chaining to handle collisions, where each index in the table\n"
                                          "contains a list of key-value pairs. Users can create a hash table, insert books by title and author,\n"
                                          ...
                                          "data management even with potential collisions. The GUI allows for easy interaction with these\n"
